...
(2) Put your reference genome in a directory and include this path in the script by "ref_genome=$". 
...
There input yeast_samples.sample_map is required here- it lists all of the files in Output/. I formatted found_vcf.tmp, so you can just copy found_vcf.tmp to yeast_samples.sample_map. Also required is intervals.list. The output is WGS_AllSamples.vcf.gz. Note: It should not be necessary to create a directory  called genomicsdb in Output/ before running this. If you get an error suggesting this, it may be because you don't have intervals.list in the current directory.
