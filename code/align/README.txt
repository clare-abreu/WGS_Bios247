...
This may be an issue with Sherlock timing out (but still telling you the job completed). Not all files may have .g.vcf.gz and .sorted.rg.bam output files. Use the scripts search_vcf.sh and search_bam.sh to search for all files in the Output folder (modified to look for your particular files). Redo alignment by running yeast_alignment_redo.sbatch, which uses missing_vcf.tmp as input. For this case it was because none of the files were missing sorted.rg.bam output files, but about 1/3 were missing g.vcf.gz output files. But if you're missing some of both, combine your missing samples into one list to use as input for yeast_alignment_redo.sbatch. Continue to run and re-run until this process is complete and you have no more missing output files.
...
There input yeast_samples.sample_map is required here- it lists all of the vcf.gz files in Output/. I formatted found_vcf.tmp, so you can just copy found_vcf.tmp to yeast_samples.sample_map. Also required is intervals.list. The output is WGS_AllSamples.vcf.gz. Note: Do NOT make a directory  called genomicsdb in Output/ before running this. If you get an error suggesting this, it may be because you don't have intervals.list in the current directory.
